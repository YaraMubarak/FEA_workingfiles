...
from feap import feap #ur feap function 
import parser 
import sys 
...
class Mesh: 
    ...
    def getpoints(self, calcium = 1 , lipid = 1 ):
        ...
        start = len(points)
        points.extend(part.draw_outer())
        end = len(points) - 1 
        markers.extend(len(part.draw_outer())*[partnum])
        facets.extend(round_trip_connect(start, end))
        partnum = partnum + 1 

        start = len(points)
        points.extend(part.draw_inner())
        end = len(points) - 1 
        markers.extend(len(part.draw_inner())*[partnum])
        facets.extend(round_trip_connect(start, end))
        partnum = partnum + 1 
    ...
    def make_mesh(self, A=1 ):
        ...
        for part in self.parts:
            if j > 2 : 
                ...
                j = j + 1            
        self.mesh_ = triangle.build(info, refinement_func=refinement_func, attributes=True) 
    ...
    def appendlabel(self,labeltextfile):
        with open(labeltextfile, "a") as myfile:
            myfile.write(str(self.maxVM))
            myfile.write("\n")
            
    def runfeap(self,feapfilename,pressure):
        flag, outfile = feap(feapfilename,pressure,nlipid = self.cal_l, ncalc = self.cal_n) 
        return flag,outfile 

    def readVM(self,outPutfile):
        p = parser.feapoutput(outPutfile) # imported object from parser.py 
        p.parse() 
        vmlist = p.finalstress.VonMisesList()
        self.maxVM =  max(vmlist) #should return labels 
...
class Structure : 
    ...
    def run(self):
        ...
        flag, self.f_o = m.runfeap(self.f_i, 90)
        if flag : 
            m.readVM(self.f_o)
...
if __name__ == "__main__":
    s = Structure(sys.argv[1],sys.argv[2],feapinputfilename=sys.argv[3] )
    s.run() 
